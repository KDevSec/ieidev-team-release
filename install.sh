...
# ── 本脚本所在目录（自包含判定 + 决策核定位都用它）──
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"

# ── 可覆盖参数（环境变量）──
# OMC 式自包含：若本目录自带完整插件（同级有 .claude-plugin/marketplace.json），默认 marketplace
# 源 = 本地路径 → `marketplace add <SCRIPT_DIR>` 装自带副本，`plugin install` 复制进缓存，
# **全程不碰 git 源码仓**。否则（curl 裸跑、无自带插件）回退 GitHub 源。
# IEIDEV_MARKETPLACE_SOURCE 永远优先（离线/本地源覆盖）。
if [ -n "${IEIDEV_MARKETPLACE_SOURCE:-}" ]; then
  MARKETPLACE_SOURCE="$IEIDEV_MARKETPLACE_SOURCE"
elif [ -f "$SCRIPT_DIR/.claude-plugin/marketplace.json" ]; then
  MARKETPLACE_SOURCE="$SCRIPT_DIR"
else
  MARKETPLACE_SOURCE="KDevSec/ieidev-team"
fi
...
# 这样 install.sh 既能在源仓 dogfood，也能被 curl 单独跑。（SCRIPT_DIR 已在顶部算好。）
INSTALLER_PYDIR=""
