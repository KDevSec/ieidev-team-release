...
4. **停人闸 + CQO 审计**：若 `step.human_gate_after` 非 null（如 `after-req`），跑完本段后**停下**——**先按 §4.5 发函 `ieidev-team:cqo-orchestrator` 审计本段过程健康度**，再把「该段交付产物 + CQO verdict」一并向人汇报，等人确认再进下一 step。（阶段交界 / flow 收尾的 checkpoint 同样按 §4.5 触发。）
   ...
   > ⚠️ **注意（见 §5 诚实债 4）**：全自动模式（`--auto` + `skipAutoPermissionPrompt`）下，human_gate 为**建议性停靠**，不保证硬停——Claude Code 在该模式下不弹权限确认，会话可能直接冲到下一段。要真正在停人闸处等人接管，请勿全自动跑，或在到达停人闸前手动切换为非全自动。
...
🔻 **诚实债 4：全自动（auto + skipAutoPermissionPrompt）下 human_gate 为建议、不保证硬停。**
§3.3 step 4 的「停人闸」是散文里的「停下」软指令——依赖 Claude Code 在到达 human_gate 时**弹出权限确认弹窗**让人点击。当使用 `--auto` 模式 + `skipAutoPermissionPrompt=true` 设置时，权限确认被全局跳过，goal 一口气冲完整条流水线（CQO 审计**仍然运行**，但并没有真正暂停等人）。**要真正在停人闸处等人接管**，请勿全自动跑（不设 `skipAutoPermissionPrompt`），或在停人闸前手动接管会话。后续计划做硬停机制（hook 级强制暂停），本版标记为「可选后续」，尚未实施。
