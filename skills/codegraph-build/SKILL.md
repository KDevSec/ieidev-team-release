...
description: 在当前项目构建代码知识图谱（先调 UA /understand 建基础图，再用 ieidev_ingestor 灌入 ieidev-team:secure-coding 的安全规范节点）。触发时机：用户说"建图谱 / 建立代码图谱 / 跑 understand / 灌安全规范 / 准备追溯"，或第一次使用 ieidev-team:codegraph-trace / ieidev-team:codegraph-impact / ieidev-team:codegraph-spec-link 但 .understand-anything/knowledge-graph.json 不存在时。
---
...
2. **ingestor inject**：把 ieidev-team:secure-coding 安全规则灌成 concept 节点（本地，0 LLM）
...
安全规范源 = 本插件 `ieidev-team:secure-coding` skill 的 references 目录。
