...
**来源**：上游 skill `ieidev-team:test-cases` 渲染规约——`是否UI自动化` + `是否API自动化` 是用例 .md 每个 `【测试用例信息】` 块的 MANDATORY 字段，明确约定 "downstream pipelines split UI ↔ API automation lanes by grepping these two exact field names"。本 skill（UI 自动化下游）按这条契约硬 gate；并行的 `ieidev-team:uicase-to-apicase` / API 自动化 skill 按 `是否API自动化=是` 分流，互不重叠。
...
| `否 / N / no / false` | ❌ **refuse 写 UI 脚本**——告知用户该用例分流通道：纯接口校验 → `ieidev-team:uicase-to-apicase` + `kdev-api-test-scaffold`；性能 / 安全 / 日志审计 → 手写 / 其它专用通道 |
| `待定 / TBD / 空白 / 字段缺失` | 🟡 **不在下游兜底**——告知用户先回 `ieidev-team:test-cases` 把字段补齐，再回来调用本 skill；本 skill 不臆测、不放行 |
...
直接使用 playwright MCP 工具序列：`browser_navigate(url)` → `browser_snapshot()` → `browser_take_screenshot()`，按需追加 `browser_console_messages()` 抓日志。拿到结果再回到本 skill 的 6 大规范继续写代码。**先用 playwright MCP 拿现场 DOM 真值 → 再回到 ieidev-team:ui-autotest 落规范**。
...
> 候选数为 0 → 告知用户"本批无 UI 自动化用例，是否走 `ieidev-team:uicase-to-apicase` 改派"；
> 候选数 > 0 → **仅对候选用例**启动下面 10 步；
> 字段空缺 / 待定 → 回 `ieidev-team:test-cases` 补字段，不兜底；
...
0. **用例池入口过滤（约束 5，强制）**：该用例的 `是否UI自动化` 字段值是什么？
   - `是` → 继续往下走
   - `否` → **refuse**，告知用户走 `ieidev-team:uicase-to-apicase` / 手写其它通道；不要因为"用户已经让我写"就放行
   - `待定 / 空白` → 让用户先回 `ieidev-team:test-cases` 补字段
