"""
STEP 0 — 环境 / 菜单 / 弹窗实测前置 一键脚本（ieidev-team:ieidev-ui-autotest skill 资产）

用途：在写测试脚本之前，登录目标系统、抓全菜单树、对每个目标页和『新增』弹窗做 UI 元素 dump，
      产出 recon/ 目录下的 4 件机器可读产物 + 截图。
...
用法：
  ...
  4. 输出落到 ./recon/ 下
  5. Claude 读完 4 类 JSON 后按 references/env-recon-bootstrap.md §阶段 4 渲染 menu_list.md
...
def main() -> None:
    ...
    print("   2. 按 references/env-recon-bootstrap.md §阶段 4 渲染 recon/menu_list.md")
    print("   3. 比对 spec 与 menu_list.md §6 差异表，更新测试用例文档头部 + 修正按钮 / 弹窗 / 列名")
    print("   4. 在测试用例文档顶部加「测试环境与导航约定」节（含 STEP_LOGIN / STEP_NAV_* 宏）")
    print("   5. 进入 SKILL.md STEP 1：tools/recon_elements.py 单页深度 dump")
