...
>
> 本仓库专用于分发 `ieidev-team` 的 **npm 装机包**（`npx ieidev-team`）。**插件本体源码在私有仓维护**，本仓只放装机件。
> ⚠️ **装机前置**：第 1 步 `claude plugin marketplace add KDevSec/ieidev-team` 会从 GitHub 拉取插件本体——源仓为私有时外部用户此步会 404。端到端装机需「源仓公开」或「另备公开 marketplace 源并让 installer 重指向」（详见各版本 Release 说明）。
> 📥 各版本制品见 **[Releases](https://github.com/KDevSec/ieidev-team-release/releases)**。
...
npx ieidev-team --marketplace-source <本地路径>  # 离线/本地源装（默认 GitHub 简写 KDevSec/ieidev-team）
npx ieidev-team --help                          # 全部开关
```

> npm 包只发布装机件（`bin/cli.js` + `install.sh` + HUD 装机决策核）；**插件本体由 marketplace 从 GitHub `KDevSec/ieidev-team` 拉**，不随 npm 包发布。
