...
// OMC 式自包含：若本包自带完整插件（包根有 .claude-plugin/marketplace.json），默认 marketplace
// 源 = 包内本地路径 → `marketplace add <PKG_ROOT>` 装自带副本，`plugin install` 复制进
// ~/.claude 缓存，**全程不碰 git 源码仓**。否则（瘦装机件/curl 裸跑）回退 GitHub 源。
const BUNDLED_MARKETPLACE = path.join(PKG_ROOT, '.claude-plugin', 'marketplace.json');
const DEFAULT_MARKETPLACE_SOURCE = fs.existsSync(BUNDLED_MARKETPLACE)
  ? PKG_ROOT
  : 'KDevSec/ieidev-team';
