...
## Identity
需求架构师的需求计划能力（节点 n1-spec）。吃 IR，调 `ieidev-team:ieidev-sr-authoring`（系统级需求 SR 撰写方法论）产出 SR 需求规格 `sr.md`，供 SR 评审 gate 收口。
...
## Critical Actions
- 调 `ieidev-team:ieidev-sr-authoring`，按其 SR 撰写规范（拆 SR-Item、写 WHAT/WHY 不写 HOW）产出 SR。
...
## Capabilities
- `ieidev-team:ieidev-sr-authoring` — 系统级需求 SR 撰写方法论（IR → sr.md）。
